...
const MAX_ENCRYPTED_PAYLOAD_FIELD_LENGTH = 16 * 1024;
...
function normalizeEncryptedPayload(value) {
  ...
  const algorithm = String(value.algorithm || "AES-GCM-256").trim();
  const purpose = String(value.purpose || "").trim().slice(0, 24);
  ...
  if (
    ...
    (purpose && !/^[a-z0-9-]+$/i.test(purpose)) ||
    (!Number.isNaN(bucket) && !Number.isInteger(bucket)) ||
    !iv ||
    !ciphertext ||
    iv.length > MAX_ENCRYPTED_PAYLOAD_FIELD_LENGTH ||
    ciphertext.length > MAX_ENCRYPTED_PAYLOAD_FIELD_LENGTH
  ) {
    return null;
  }
  ...
  return {
    ...
    bucket: Number.isInteger(bucket) ? bucket : undefined,
    iv,
    ciphertext
  };
}
...
function hashMeetingPassword(password, salt) {
  return crypto.scryptSync(password, salt, 32).toString("hex");
}

function createPasswordRecord(password) {
  const salt = crypto.randomBytes(16).toString("hex");
  const hash = hashMeetingPassword(password, salt);
  return { salt, hash };
}

function verifyMeetingPassword(password, record) {
  if (!record?.salt || !record?.hash) {
    return false;
  }

  const expected = Buffer.from(record.hash, "hex");
  const actual = Buffer.from(hashMeetingPassword(password, record.salt), "hex");
...
async function createRoom(ownerName, roomTitle, roomPassword) {
  ...
  return withFileLock(FILES.rooms, async () => {
    ...
    const passwordRecord = createPasswordRecord(roomPassword);
...
async function joinRoom(roomCode, userName, roomPassword) {
  ...
  return withFileLock(FILES.rooms, async () => {
    ...
    if (!verifyMeetingPassword(roomPassword, room)) {
...
async function handleApi(request, response, urlObject) {
  ...
  if (request.method === "GET" && pathname === "/api/config") {
    sendJson(response, 200, {
      ...
      endToEndEncryption: {
        enabled: true,
        mode: "client-managed-room-key"
      }
    });
    return;
  }

  if (request.method === "GET" && pathname === "/api/graph/profile") {
    sendJson(response, 200, {
      appName: "Graph",
      release: "2.0-minimal",
      securityTier: "very-high",
      externalDependency: "none",
      storage: "local-json",
      networkScope: "lan-only"
    });
    return;
  }
  ...
  if (request.method === "POST" && pathname === "/api/rooms") {
    ...
    assert(roomPassword.length >= 6, "رمز جلسه باید حداقل 6 کاراکتر باشد.");
  ...
  if (request.method === "POST" && pathname === "/api/rooms/join") {
    ...
    assert(normalizePassword(body.roomPassword).length >= 6, "رمز جلسه معتبر نیست.");
    const result = await joinRoom(body.roomCode, body.userName, body.roomPassword);

    if (result.error) {
      if (result.error === "invalid_password") {
        sendJson(response, 403, { error: "رمز جلسه اشتباه است." });
        return;
      }
  ...
  if (request.method === "GET" && pathname === "/api/rooms/detail") {
    ...
    if (!participant) {
      sendJson(response, 403, { error: "مجوز مشاهده وضعیت اتاق معتبر نیست." });
      return;
    }
  ...
  if (request.method === "POST" && pathname === "/api/signals") {
    ...
    const signal = {
      ...
      payload: body.encrypted ? normalizeEncryptedPayload(body.payload) : body.payload
    };

    assert(signal.payload, "محتوای سیگنال معتبر نیست.");
    assert(byteLengthOfJson(signal.payload) <= MAX_SIGNAL_PAYLOAD_BYTES, "حجم سیگنال بیش از حد مجاز است.");
  ...
  if (request.method === "POST" && pathname === "/api/messages") {
    ...
    const encryptedPayload = normalizeEncryptedPayload(body.encryptedPayload);
    ...
    const message = await addMessage(roomCode, {
      sessionId: body.sessionId,
      sender: encryptedPayload ? undefined : participant.name,
      text: encryptedPayload ? undefined : text,
